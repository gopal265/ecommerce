...
import { clearErrors, registerMail, loginBYPassword, getuser } from '../../actions/auth'
...
const Login = () => {
  ...
  const continues = () => {
    ...
    if (email.trim() === "") return par.innerHTML = "Please Entert the Email Address";
  ...
  useEffect(() => {
    ...
    if (login && message === "Successfully Loggedin" && user) {
      dispatch(getuser())
      navigate('/')
    }
  ...
  return (
    ...
    <>
      <NavBar />
      {  error &&
         <div class="alert alert-danger center login-error" role="alert" id='backenderror'>
         {error}
      ...
      <div className='full-width full-height center '>
        <div className=' login-wrapper' >
          ...
          <div className='p-5'>
            ...
            {login && 
            <div>
                 <input type='password' name='password' className='login-email' onChange={(e) => setPassword(e.target.value)} placeholder='Enter Password' />
